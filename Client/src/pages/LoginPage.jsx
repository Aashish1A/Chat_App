    ...
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
        ...
        <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
            <h2 className='font-medium text-2xl flex justify-between items-center'>
                {currentState}
                {isDataSubmitted && (
                   <img onClick={() => setIsDataSubmitted(false)} src={assets.arrow_icon} alt="arrow_icon" className='w-5 cursor-pointer' /> 
                )}
            </h2>
            ...
            <button type='submit' className='bg-gradient-to-r from-purple-400 to-violet-600 text-white border-none text-sm font-light py-2 px-6 rounded-full cursor-pointer'>{currentState === "Sign up" ? "Create Account" : "Login"}</button>
            ...
            <div className='flex flex-col gap-2'>
                {currentState === "Sign up" ? (
                    ...
                    <p className='text-sm text-gray-600'>Don't have an account? <span onClick={() => {setCurrentState("Sign up"); setIsDataSubmitted(false)}} className='font-medium text-violet-500 cursor-pointer'>Sign up</span></p>
                )}
            </div>
        </form>
    </div>
