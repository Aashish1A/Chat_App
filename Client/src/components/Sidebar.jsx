  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ""}`}>
      <div className='pb-5'>
        ...
        {/* Search Bar */}
        <div className='bg-[#282142] rounded-full flex items-center gap-2 px-3 py-4 mt-5'>
            <img src={assets.search_icon} alt="search_icon" className='max-w-3 cursor-pointer' />
      ...
      {/* User List */}
        <div className='flex flex-col'>
            {filteredUsers.map((user, index) => (
                <div key={index} onClick={() => {setSelectedUser(user); setUnseenMessages((prev) => ({ ...prev, [user._id]: 0 })); }} className={`flex items-center gap-2 p-2 hover:bg-[#282142] rounded-md cursor-pointer relative max-sm:text-sm ${selectedUser?._id === user._id ? 'bg-[#282142]/50' : ''}`}>
