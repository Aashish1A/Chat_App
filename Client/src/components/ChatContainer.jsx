...
const ChatContainer = () => {
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
        ...
        {/* Chat Header */}
        <div className='flex items-center gap-3 p-3 mx-4 border-b border-stone-500'>
            <img src={selectedUser.profilePicture || assets.avatar_icon} alt="profile_martin" className='w-8 rounded-full' />
        ...
        <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
            {messages.map((msg,index) => (
                <div key={index} className={`flex items-end gap-2 justify-end ${msg?.senderId !== authUser._id && "flex-row-reverse"}`}>
                    {msg?.image ? (
                        <img src={msg.image} alt="message_image" className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8' />
                    ) : (
                    ...
                    <div className='text-center text-xs'>
                       ...
                       <p className='text-gray-500'>{msg?.createdAt ? formatMessageTime(msg.createdAt) : 'Now'}</p>
                    </div>
                </div>
            ))}
        ...
        <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
            <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
                <input onChange={(e) => setInput(e.target.value)} value={input} onKeyDown={(e) => e.key === "Enter" ? handleSendMessage(e) : null} type="text" placeholder='Send a message' className='flex-1 text-sm p-3 border-none outline-none text-white placeholder-gray-400' />
    ...
    <div className='flex justify-center items-center flex-col gap-2 text-gray-500 bg-white/10 max-md:hidden'>
        <img onClick={handleSendMessage} src={assets.logo_icon} alt="logo_icon" className='max-w-16' />
